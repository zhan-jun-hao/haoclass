...
public class PaymentSuccessListener {
     ...
     *
     * @param message 元数据
     * @param channel 与rabbitMQ交流的通道
     * @throws IOException
     */
    @RabbitListener(queues = PayRabbitConstants.PAYMENT_SUCCESS_QUEUE)
    public void onMessage(Message message, Channel channel) throws IOException {
        long tag = message.getMessageProperties().getDeliveryTag();
        Object traceId = message.getMessageProperties().getHeaders()
                .get(MqMessageHeadersKeyConstants.TRACE_ID);
        ...
        try {
            ...
            log.error("支付成功消息消费失败, paymentNo: {}", paymentNo, e);
    ...
    private String getMessageBody(Message message) {
        ...
        if (body == null || body.length == 0) {
            throw new IllegalStateException("支付成功消息体不能为空");
        }
